export default async function handler(req, res) {
  ...
  try {
    ...
    if (!apikey) return res.status(500).json({ error: "Missing ATTOM_API_KEY" })
    ...
    const { r: propertyResp, j: propertyData } = await getJson(propertyUrl)
    ...
    if (attomId) {
      ...
      const { r: avmByIdResp, j: avmByIdData } = await getJson(avmByIdUrl)
    ...
    // 3) fallback avm/detail by address
    if (avmValue == null) {
      const avmByAddressUrl =
        "https://api.gateway.attomdata.com/propertyapi/v1.0.0/avm/detail?address=" +
        encodeURIComponent(address)

      const { r: avmByAddressResp, j: avmByAddressData } = await getJson(avmByAddressUrl)
